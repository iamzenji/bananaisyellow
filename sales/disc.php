	...
	<div class="main-content"><?php include('sidebar.php'); ?>
		...
		<div>
			<div class="header">
				...
				<h1><i style="margin: 1rem;" class='bx bxs-discount bx-tada'></i> Discount's</h1>
			</div>
			...
			<div style="box-shadow: 0 5px 15px rgba(0, 0, 0, 0.35);width: 86vw; padding: 1rem 1rem 2rem 1rem; margin: 2rem 2rem 4rem 3rem;">
				<div style="position: absolute;     margin-left: 2rem;">
                    ...
                    <button onclick="location.href='refunded.php'" type="button" class="btn btn-warning"><i class='bx bxs-file-export bx-tada' ></i> Refunded Item's</button>
						...
						<div class="item-map">
							<table class="table table-striped table-hover mapping" style="width: 100%;">
								...
								<tbody>
									...
									SUM(DISTINCT CASE WHEN type = 'PWD' THEN total_dis ELSE 0 END) AS pwd,
									SUM( CASE WHEN type = 'Senior' THEN total_dis ELSE 0 END) AS senior,
									SUM(CASE WHEN pay_method = 'Cash' THEN total_amount ELSE 0 END) AS cash,
									SUM(CASE WHEN pay_method = 'Gcash' THEN total_amount ELSE 0 END) AS gcash FROM transact  WHERE cashier_id = $cashier GROUP BY year ORDER BY date; ";
									...
									?>
											<tr>
												...
												<td>₱ <?= number_format($row['pwd'], 2) ?></td>
												<td>₱ <?= number_format($row['senior'], 2) ?></td>
						...
						<div class="item-map">
							<table class="table table-striped table-hover mapping" style="width: 100%;">
								...
								<tbody>
									...
									SUM(total_dis) as discount,
									SUM(sub_total) as sub_total,
									SUM( CASE WHEN type = 'PWD' THEN total_dis ELSE 0 END) AS pwd,
									SUM( CASE WHEN type = 'Senior' THEN total_dis ELSE 0 END) AS senior,
									SUM(CASE WHEN pay_method = 'Cash' THEN total_amount ELSE 0 END) AS cash,
									SUM(CASE WHEN pay_method = 'Gcash' THEN total_amount ELSE 0 END) AS gcash FROM transact  WHERE cashier_id = $cashier GROUP BY year, month ORDER BY date;";
									$query_run = mysqli_query($connection, $query);
						...
						<div class="item-map">
							<table class="table table-striped table-hover mapping" style="width: 100%;">
								...
								<tbody>
									...
									SUM( CASE WHEN type = 'Senior' THEN total_dis ELSE 0 END) AS senior,
									SUM(CASE WHEN pay_method = 'Cash' THEN total_amount ELSE 0 END) AS cash,
									SUM(CASE WHEN pay_method = 'Gcash' THEN total_amount ELSE 0 END) AS gcash FROM transact  WHERE cashier_id = $cashier GROUP BY year, month, day ORDER BY date;";
									$query_run = mysqli_query($connection, $query);
