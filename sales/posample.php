	...
	<div class="main-content"><?php include('sidebar.php'); ?>
		<div class="">
			...
			<div class="wrap" id="wrap">
				...
				<div id="scan" class="input-group input-group-sm flex-nowrap">
					<span class="input-group-text bg-warning" id="addon-wrapping">scan</span>
					<input id="scanner" type="text" class="form-control" placeholder="scan barcode" aria-describedby="addon-wrapping" autofocus>
				</div>
							...
							?>
									<tr>
										...
										<td><?php
											...
											?></td>
										<td style="text-align: right;"> ₱ <?php $price = $row['unit_cost'] + ($row['unit_cost'] * .15);
			...
			<section class="cart">
				...
				<table class="table" style="width:100%" id="laman_cart">
					<thead class="table-warning sticky-top" style="z-index: 0;">
						<th>Name</th>
						<th>Price</th>
						...
						if (mysqli_num_rows($query_run) > 0) { ?>
							<?php foreach ($query_run as $row) : ?>
								<tr id="<?php echo $row["cart_id"]; ?>">
									...
									<td> ₱ <?php $price = $row['unit_cost'] + ($row['unit_cost'] * .15);
			...
			<section class="checkout">
				<div class="card" style="text-align:center; width: 19rem; ">
					<div class="card-body">
				...
				<div style="margin-left: 1rem;">
					<div id="con"><button style="width: 13rem; margin-bottom:2rem; margin-top:1.6rem;" id="deleted" class="btn btn-danger btn-lg">Remove all</button></div>
					<div id="total">
		...
		<div class="modal-dialog">
			<div class="modal-content">
								...
								?>
										...
										<tr>
											<td style="text-align: left;"><?php echo $fetch_cart['category'] . " " .  $fetch_cart['brand'] . " " .  $fetch_cart['type'] . " " .  $fetch_cart['unit']; ?>
												...
												<?php $price = $fetch_cart['unit_cost'] + ($fetch_cart['unit_cost'] * .15); ?>
											<td colspan="2"><?php echo "$fetch_cart[qty]" . '/' . "$fetch_cart[scale]"; ?> </td>
											<td colspan="2">₱ <?php $sub_total = round($price) * $fetch_cart['qty'];
				...
				<div class="modal-footer">
					...
					<button id="print" class="btn bg-success">print</button>
					<!-- <a href="#" id="save" class="btn btn-primary">Update</a> -->
					<button type="button" id="save" class="btn btn-warning">done</button>
				</div>
...
<script>
	...
	$(document).on('input', '#scanner', function() {

		let barcode = $(this).val();
		$.ajax({
			type: "GET",
			url: "code.php?barcode=" + barcode,
			success: function(response) {
